# sbatch --job-name=casa_22A-020_KA --account=astronomy-dept --qos=astronomy-dept-b --nodes=1 --ntasks=16 --mem=256gb --time=96:00:00 --output=/blue/adamginsburg/adamginsburg/logs/VLA-22A-020_sgrb2_KA_%j.log --wrap "xvfb-run -a /orange/adamginsburg/casa/casa-6.6.6-17-pipeline-2025.1.0.35-py3.10.el8/bin/casa --pipeline -c /orange/adamginsburg/sgrb2/22A-020/code/imaging_Kband_Aarray.py"
...
listobs(vis[0], listfile='Kband_Aarray.listobs', overwrite=True)

contspw = [18,19,20,21,22,23,24,25,30,31,32,33,34,46,47,48]

logprint("Step 1: Preliminary imaging")
for robust in (0, 2):
    if not os.path.exists(f'Kband_Aarray.center.robust{robust}.continuum.big-coarse.liteclean.psf.tt0'):
        tclean(vis=vis,
               imagename=f'Kband_Aarray.center.robust{robust}.continuum.big-coarse.liteclean',
...
startmodel = ''
for robust in (2, 0):
    imagename = f'Kband_Aarray.center.robust{robust}.continuum.deepclean'
...
caltable = 'Kband_Aarray.center.pcal1'
caltable_nocombine = 'Kband_Aarray.center.pcal.nocombine'
...
for robust in (0, 2):
    imagename = f'Kband_Aarray.center.robust{robust}.continuum.deepclean.selfcal'
    if not os.path.exists(f'{imagename}.psf.tt0'):
        tclean(vis=[vis_selfcal_contavg],
               ...
               mask='clean_mask.crtf',
               savemodel='modelcolumn')
    imagename = f'Kband_Aarray.center.robust{robust}.continuum.deepclean.selfcal.preaveraged'
    if not os.path.exists(f'{imagename}.psf.tt0'):
        tclean(vis=[vis_contavg],
               datacolumn='corrected',
               imagename=imagename,
