...
for spw in (range(48,2,-1)):
    if spw in contspw:
        continue
    if not os.path.exists(f'KbandDarray.sgrb2n.spw{spw}.liteclean.psf'):
        tclean(vis=vis,
               imagename=f'KbandDarray.sgrb2n.spw{spw}.liteclean',
    ...
    if not os.path.exists(f'Kband.sgrb2m.spw{spw}.liteclean.psf'):
        tclean(vis=vis,
               imagename=f'KbandDarray.sgrb2m.spw{spw}.liteclean',
...
if not os.path.exists(f'KbandDarray.center.robust{robust}.continuum.big-coarse.liteclean.psf'):
    tclean(vis=vis,
           ...
           niter=1000, spw=",".join(map(str,contspw)), field='sgr b2b', imsize=[500],
